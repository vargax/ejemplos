...
for evaluacion in evaluaciones:
    ...
    if len(evaluacion.split('_')) == 2:  # El nombre que se coloca a mano es EC_login.txt
        ...
        alumno = evaluacion.split('_')[
            1]  # Obteniendo el login del alumno que envió la evaluación del nombre del archivo
    ...
    for companero in evaluacion:
        if companero.startswith(
                'PNLZD'):  # Si el archivo se debió corregir a mano, la última línea empieza por PNLZD y separado por , la razón
            penalizacion = True
        ...
        try:
            ...
            penalizacion = True
            razon += 'ERR :: ' + alumno + ' :: puntos asignados a ' + login + ' no parseables a float: ' + \
                     companero.split(',')[1]
            penalizaciones[alumno] = razon
